// FaceLove Pro - Premium Social Platform
...
// ========== CONFIGURATION ==========
const SESSION_SECRET = 'facelove_gold_secret_2026';
const UPLOAD_DIR = './uploads';
if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR);

// Twilio config (optional - use env variables)
...
if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN) {
  ...
  } catch (e) { console.log('⚠️ Twilio not configured'); }
}
...
function readDB() {
  ...
  catch (e) { return { users: [], otps: [], posts: [], friendships: [], messages: [], notifications: [] }; }
}
...
async function sendOTP(phone, otp) {
  ...
  try {
    await twilioClient.messages.create({
      body: `Your FaceLove Pro verification code: ${otp}`,
...
app.get('/login', (req, res) => {
  ...
  res.send(`
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>FaceLove Pro - Login</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
    ...
    <body>
      <div class="auth">
        ...
        <div class="links">
          ...
          <a href="/forgot-password"><i class="fas fa-key"></i> Forgot?</a>
        </div>
      </div>
    </body>
    </html>
...
app.get('/register', (req, res) => {
  res.send(`
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>FaceLove Pro - Register</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
    ...
    <body>
      <div class="auth">
        <div class="brand">
          <i class="fas fa-crown"></i>
          <h1>Face<span>Love</span> Pro</h1>
          <p>Join the premium community</p>
        </div>
        <form action="/register" method="POST">
          ...
          <div class="info-box"><i class="fas fa-info-circle"></i> A verification code will be sent via WhatsApp</div>
          <button type="submit" class="btn"><i class="fas fa-user-plus"></i> Create Account</button>
        </form>
        <div class="links">
          Have an account? <a href="/login"><i class="fas fa-sign-in-alt"></i> Sign In</a>
        </div>
      </div>
    </body>
    </html>
...
app.post('/register', async (req, res) => {
  ...
  if (db.users.find(u => u.username === username)) {
    return res.send('<script>alert("Username already taken"); window.location="/register";</script>');
  }
  ...
  req.session.tempUser = { username, phone, password };
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Verify - FaceLove Pro</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
        ...
        .auth .brand i { font-size: 44px; color: #d4af37; margin-bottom: 12px; text-shadow: 0 0 40px rgba(212,175,55,0.3); }
        .auth .brand h1 { color: #fff; font-size: 28px; font-weight: 700; letter-spacing: 1px; }
        .auth .brand h1 span { color: #d4af37; }
        ...
        .auth .form-group input:focus { border-color: #d4af37; background: rgba(40,40,40,0.9); }
        .auth .btn {
          width: 100%;
          padding: 16px;
          background: linear-gradient(135deg, #d4af37, #f9d976);
...
app.post('/verify-otp', (req, res) => {
  ...
  const newUser = {
    ...
    gold: false,           // gold badge flag
    joined: new Date().toISOString(),
    bio: 'Welcome to FaceLove Pro!',
    avatar: '',
    cover: ''
  };
...
app.get('/dashboard', (req, res) => {
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .top-nav .brand i { color: #d4af37; font-size: 22px; }
        ...
        .top-nav .actions .user-badge .name {
          font-size: 13px;
          font-weight: 500;
          color: #ccc;
        }
        .top-nav .actions .user-badge .gold-badge {
          color: #d4af37;
          font-size: 14px;
        ...
        .card:hover {
          ...
          box-shadow: 0 8px 40px rgba(212,175,55,0.05);
        }
        .card .title {
          font-size: 16px;
          font-weight: 700;
        ...
        .post-box .actions .media-options label:hover { color: #d4af37; }
        .post-box .actions .media-options .file-name {
          font-size: 12px;
          color: #666;
          max-width: 120px;
          overflow: hidden;
        ...
        .feed-post:hover {
          ...
          box-shadow: 0 8px 30px rgba(212,175,55,0.04);
        }
        .feed-post .header {
          display: flex;
          align-items: center;
        ...
        .feed-post .header .info .name .gold-icon {
          color: #d4af37;
          font-size: 14px;
        ...
        .feed-post .comments input:focus {
          border-color: #d4af37;
          background: rgba(40,40,40,0.8);
        }
        ...
        .friend-item .avatar .gold-badge-sm {
          position: absolute;
          bottom: -2px;
          right: -2px;
          font-size: 12px;
        ...
        .friend-item .info .name .gold-icon-sm {
          color: #d4af37;
          font-size: 12px;
        }
        ...
        .friend-item .actions .btn-sm.primary {
          background: linear-gradient(135deg, #d4af37, #f9d976);
          color: #0a0a0a;
        }
        .friend-item .actions .btn-sm.primary:hover {
          transform: translateY(-2px);
          box-shadow: 0 6px 20px rgba(212,175,55,0.3);
        }
        .friend-item .actions .btn-sm.success {
          background: #27ae60;
          color: #fff;
        }
        .friend-item .actions .btn-sm.success:hover { background: #2ecc71; }
        .friend-item .actions .btn-sm.danger {
          background: #c0392b;
          color: #fff;
        }
        ...
        .friend-item .actions i:hover { color: #d4af37; }
        ...
        .admin-item .info .name .gold-icon-sm {
          color: #d4af37;
          font-size: 14px;
        }
        .admin-item .info .sub {
        ...
        .admin-item .actions .btn-xs.danger {
          background: #c0392b;
          color: #fff;
        }
        .admin-item .actions .btn-xs.danger:hover { background: #e74c3c; }
        .admin-item .actions .btn-xs.success {
          background: #27ae60;
          color: #fff;
        }
        .admin-item .actions .btn-xs.success:hover { background: #2ecc71; }
        .admin-item .actions .btn-xs.gold {
          background: #d4af37;
          color: #0a0a0a;
        }
        .admin-item .actions .btn-xs.gold:hover {
          background: #f9d976;
          transform: translateY(-2px);
        }
    ...
    <body>
      <!-- TOP NAV -->
      <nav class="top-nav">
        <div class="brand"><i class="fas fa-crown"></i>Face<span>Love</span> Pro</div>
        <div class="actions">
      ...
      <div class="content">
        ...
        <div id="page-feed" class="page active">
          <div class="card post-box">
            <textarea id="postText" placeholder="What's on your mind? Share with the community..." rows="3"></textarea>
            <div class="actions">
              <div class="media-options">
                <label for="imageInput" title="Add Image"><i class="fas fa-image"></i></label>
                <label for="videoInput" title="Add Video"><i class="fas fa-video"></i></label>
        ...
        <div id="page-profile" class="page">
          <div class="card">
            <div class="profile-header">
              ...
              <div class="bio">${user.bio || 'Welcome to FaceLove Pro!'}</div>
              <div class="stats">
              ...
              <button class="edit-btn" style="margin-top:8px;" onclick="openChangePassword()"><i class="fas fa-key"></i> Change Password</button>
            </div>
          </div>
        ...
        <div id="page-chat" class="page">
          <div class="card">
            ...
            <div id="chatDetail" style="display:none;">
              ...
              <button onclick="closeChat()" style="margin-top:10px;background:none;border:none;color:#d4af37;cursor:pointer;font-size:13px;"><i class="fas fa-arrow-left"></i> Back</button>
            </div>
          </div>
        </div>
        ...
        <div id="page-admin" class="page">
          <div class="card">
            ...
            <p style="color:#888;font-size:13px;margin-bottom:16px;">Manage your community with premium control</p>
      ...
      <script>
        ...
        async function loadFeed() {
          try {
            ...
            document.getElementById('feedContainer').innerHTML = posts.length ?
              ...
              '<div class="empty"><i class="fas fa-inbox"></i><p>No posts yet. Be the first!</p></div>';
          ...
          return \`
            <div class="feed-post">
              ...
              <div class="comments" id="comments-\${p.id}" style="display:none;">
                ...
                <input type="text" placeholder="Write a comment..." onkeypress="if(event.key==='Enter') addComment('\${p.id}', this.value); this.value='';">
              </div>
            </div>
          \`;
        }
        ...
        function sharePost(postId) {
          ...
          if (navigator.share) {
            navigator.share({ title: 'FaceLove Pro Post', text: 'Check this out!', url: url });
          } else {
        ...
        async function searchContent() {
          ...
          try {
            ...
            if (data.users && data.users.length) {
              html += '<div style="margin-bottom:12px;"><strong style="color:#d4af37;"><i class="fas fa-users"></i> Users</strong></div>';
              html += data.users.map(u =>
                '<div class="friend-item"><div class="avatar">'+u.username.charAt(0).toUpperCase()+(u.gold ? '<span class="gold-badge-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="info"><div class="name">'+u.username+(u.gold ? ' <span class="gold-icon-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="status">'+u.phone+'</div></div><div class="actions"><i class="fas fa-user-plus" onclick="sendFriendRequest(\\''+u.id+'\\')" title="Add Friend"></i></div></div>'
              ).join('');
            }
            if (data.posts && data.posts.length) {
              if (html) html += '<div style="margin:16px 0 8px;"><strong style="color:#d4af37;"><i class="fas fa-newspaper"></i> Posts</strong></div>';
        ...
        async function loadFriends() {
          try {
            ...
            document.getElementById('friendRequests').innerHTML = data.requests.length ?
              data.requests.map(r =>
                '<div class="friend-item"><div class="avatar">'+r.username.charAt(0).toUpperCase()+(r.gold ? '<span class="gold-badge-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="info"><div class="name">'+r.username+(r.gold ? ' <span class="gold-icon-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="status"><i class="fas fa-clock" style="color:#f1c40f;"></i> Pending</div></div><div class="actions"><button class="btn-sm success" onclick="acceptFriend(\\''+r.id+'\\')">Accept</button><button class="btn-sm danger" onclick="rejectFriend(\\''+r.id+'\\')">Decline</button></div></div>'
              ).join('') :
            ...
            document.getElementById('suggestedUsers').innerHTML = data.suggested.length ?
              data.suggested.map(u =>
                '<div class="friend-item"><div class="avatar">'+u.username.charAt(0).toUpperCase()+(u.gold ? '<span class="gold-badge-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="info"><div class="name">'+u.username+(u.gold ? ' <span class="gold-icon-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="status">'+u.phone+'</div></div><div class="actions"><button class="btn-sm primary" onclick="sendFriendRequest(\\''+u.id+'\\')">Add</button></div></div>'
              ).join('') :
        ...
        async function loadChatList() {
          try {
            ...
            document.getElementById('chatList').innerHTML = data.length ?
              data.map(u =>
                '<div class="friend-item" onclick="openChat(\\''+u.id+'\\')"><div class="avatar">'+u.username.charAt(0).toUpperCase()+(u.gold ? '<span class="gold-badge-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="info"><div class="name">'+u.username+(u.gold ? ' <span class="gold-icon-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="status"><i class="fas fa-comment"></i> Click to chat</div></div></div>'
              ).join('') :
        ...
        async function loadAdmin() {
          try {
            ...
            \`;
            let html = '';
            html += '<div style="margin:16px 0 8px;"><strong style="color:#d4af37;"><i class="fas fa-users"></i> Users</strong></div>';
            data.users.forEach(u => {
              html += '<div class="admin-item"><div class="info"><div class="name">'+u.username+(u.gold ? ' <span class="gold-icon-sm"><i class="fas fa-crown"></i></span>' : '')+'</div><div class="sub">'+u.phone+' • Joined '+new Date(u.joined).toLocaleDateString()+'</div></div><div class="badge '+(u.active !== false ? 'active' : 'inactive')+'">'+(u.active !== false ? 'Active' : 'Inactive')+'</div><div class="actions"><button class="btn-xs '+(u.gold ? 'gold' : '')+'" onclick="adminToggleGold(\\''+u.id+'\\', '+(u.gold ? 'false' : 'true')+')">'+(u.gold ? 'Remove Gold' : 'Grant Gold')+'</button><button class="btn-xs '+(u.active !== false ? 'danger' : 'success')+'" onclick="adminToggleUser(\\''+u.id+'\\', '+(u.active !== false ? 'false' : 'true')+')">'+(u.active !== false ? 'Deactivate' : 'Activate')+'</button></div></div>';
            });
            html += '<div style="margin:20px 0 8px;"><strong style="color:#d4af37;"><i class="fas fa-newspaper"></i> Posts</strong></div>';
...
app.get('/forgot-password', (req, res) => {
  res.send(`
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>FaceLove Pro - Forgot Password</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
...
app.post('/forgot-password', async (req, res) => {
  ...
  res.send(`
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Reset Password - FaceLove Pro</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body {
...
app.get('/api/search', (req, res) => {
  ...
  const enriched = posts.map(p => {
    const user = db.users.find(u => u.id === p.userId);
    return { ...p, userName: user ? user.username : 'User', userGold: user ? user.gold || false : false };
  });
...
app.listen(PORT, () => {
  ...
  console.log('🌟 Premium Social Platform');
});
