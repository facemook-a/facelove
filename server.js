...
const db = { users: [], posts: [], messages: [], quizzes: [] };
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (findUserByUsername(username)) return res.status(400).json({ error: 'Username taken' });
...
app.get('/api/posts/feed', auth, (req, res) => {
  ...
  const enriched = posts.map(p => {
    ...
    return { ...p, userId: user ? { username: user.username, avatar: user.avatar } : null };
  });
  res.json(enriched);
});
...
.lang-select button{background:none;border:1px solid #ddd;border-radius:20px;padding:2px 10px;font-size:0.75rem;cursor:pointer;color:#888;transition:0.2s}
.lang-select button.active{background:#ff4757;color:#fff;border-color:#ff4757}
...
<h4 style="text-align:center;margin-bottom:16px" id="login-title">تسجيل الدخول</h4>
<input type="email" id="login-email" placeholder="البريد الإلكتروني">
<input type="password" id="login-pass" placeholder="كلمة المرور">
<button onclick="handleLogin()" id="login-btn">دخول</button>
<div class="toggle" onclick="toggleAuth()" id="toggle-link">ليس لديك حساب؟ سجل الآن</div>
</div>
<div class="auth-form" id="register-form" style="display:none">
<h2 style="text-align:center;color:#ff4757"><i class="fas fa-heart"></i> FaceLove</h2>
<h4 style="text-align:center;margin-bottom:16px" id="register-title">إنشاء حساب</h4>
...
<button onclick="handleRegister()" id="register-btn">تسجيل</button>
<div class="toggle" onclick="toggleAuth()" id="toggle-link2">لديك حساب؟ سجل دخول</div>
</div>
</div>
...
<button class="active" data-lang="ar" onclick="setLang('ar')">ع</button>
...
<script>
// ====== متغيرات ======
let currentUser = null, socket = null, activeChat = null, token = localStorage.getItem('token');
let lang = 'ar';

// ====== النصوص متعددة اللغات ======
const L = {
  ar: {
    loginTitle: 'تسجيل الدخول', loginBtn: 'دخول', toggle: 'ليس لديك حساب؟ سجل الآن',
    regTitle: 'إنشاء حساب', regBtn: 'تسجيل', toggle2: 'لديك حساب؟ سجل دخول',
    postPlaceholder: 'شارك ما يخطر ببالك...', postBtn: 'نشر', addImage: 'إضافة صورة',
    chatPlaceholder: 'اكتب رسالة...', noChats: 'لا توجد محادثات', editProfile: 'تعديل الملف',
    verified: 'موثق', requestVerify: 'طلب توثيق', quizTitle: 'اختبار التوافق',
    adminUsers: 'المستخدمين', adminPosts: 'المنشورات', adminMessages: 'الرسائل'
  },
  en: {
    loginTitle: 'Sign In', loginBtn: 'Sign In', toggle: 'No account? Sign up',
    regTitle: 'Create Account', regBtn: 'Register', toggle2: 'Have an account? Sign in',
    postPlaceholder: 'What\'s on your mind?', postBtn: 'Post', addImage: 'Add Image',
    chatPlaceholder: 'Type a message...', noChats: 'No chats yet', editProfile: 'Edit Profile',
    verified: 'Verified', requestVerify: 'Request Verification', quizTitle: 'Compatibility Quiz',
    adminUsers: 'Users', adminPosts: 'Posts', adminMessages: 'Messages'
  },
  fr: {
    loginTitle: 'Connexion', loginBtn: 'Se connecter', toggle: 'Pas de compte ? Inscrivez-vous',
    regTitle: 'Créer un compte', regBtn: 'S\'inscrire', toggle2: 'Vous avez un compte ? Connectez-vous',
    postPlaceholder: 'Quoi de neuf ?', postBtn: 'Publier', addImage: 'Ajouter une image',
    chatPlaceholder: 'Écrivez un message...', noChats: 'Aucune discussion', editProfile: 'Modifier le profil',
    verified: 'Vérifié', requestVerify: 'Demander la vérification', quizTitle: 'Quiz de compatibilité',
    adminUsers: 'Utilisateurs', adminPosts: 'Publications', adminMessages: 'Messages'
  }
};

function t(key) { return L[lang]?.[key] || L.ar[key] || key; }

function setLang(l) {
  lang = l;
  document.querySelectorAll('.lang-select button').forEach(b => b.classList.toggle('active', b.dataset.lang === l));
  updateUI();
...
async function handleRegister() {
  ...
  try {
    const res = await fetch('/api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, email, password: pass }) });
    const data = await res.json();
    if (res.ok) { token = data.token; localStorage.setItem('token', token); currentUser = data.user; initApp(); }
...
function initApp() {
  ...
  socket.on('receiveMessage', (msg) => {
    if (activeChat && (msg.senderId._id === activeChat || msg.receiverId._id === activeChat)) appendMsg(msg);
  });
  socket.on('newPost', () => { if (document.getElementById('page-feed').classList.contains('active')) loadFeed(); });
  navigate('feed');
  updateUI();
}
...
function navigate(page) {
  ...
  const navMap = { feed: 'nav-feed', chat: 'nav-chat', profile: 'nav-profile', admin: 'nav-admin' };
    ...
    c.innerHTML = posts.map(p => \`
      <div class="card">
        <div class="post-header">
          <div class="avatar">\${p.userId?.username?.[0]||'?'}</div>
          <div class="info"><div class="name">\${p.userId?.username||'مستخدم'}</div><div class="time">\${new Date(p.createdAt).toLocaleString()}</div></div>
        </div>
        <div>\${p.text}</div>
...
async function createPost() {
  ...
  try {
    ...
    if (res.ok) { document.getElementById('post-text').value = ''; document.getElementById('post-image').value = ''; document.getElementById('img-name').textContent = ''; loadFeed(); }
...
async function loadChatList() {
  try {
    ...
    c.innerHTML = users.map(u => \`<div class="chat-item" onclick="openChat('\${u._id}','\${u.username}')"><div class="avatar">\${u.username?.[0]||'?'}</div><div><strong>\${u.username}</strong></div></div>\`).join('') || '<p style="color:#999;text-align:center;padding:20px">'+t('noChats')+'</p>';
...
async function loadMessages(id) {
  try {
    ...
    c.innerHTML = msgs.map(m => \`<div class="msg \${m.senderId?._id===currentUser.id?'sent':''}">\${m.senderId?._id===currentUser.id?'':'<strong>'+m.senderId?.username+':</strong> '}\${m.text}</div>\`).join('');
...
async function loadProfile() {
  try {
    ...
    const my = posts.filter(p => p.userId?._id === user.id);
    document.getElementById('profile-posts').innerHTML = my.map(p => \`<div class="card"><p>\${p.text}</p>\${p.image?'<img src="'+p.image+'" class="post-image">':''}</div>\`).join('') || '<p style="color:#999;text-align:center;padding:20px">لا توجد منشورات</p>';
  } catch (e) { console.error(e); }
}
...
async function submitQuiz() {
  const answers = [];
  for (let i=0;i<5;i++) { const sel = document.querySelector('input[name="q'+i+'"]:checked'); if (!sel) return alert('أجب على جميع الأسئلة'); answers.push(parseInt(sel.value)); }
  try {
...
async function loadAdmin() {
  ...
  try {
    ...
    document.getElementById('admin-users').innerHTML = users.map(u =>
      '<div class="user-row"><span>'+u.username+(u.verified?' ✅':'')+'</span><span>'+(u.isActive?'نشط':'محظور')+'</span><span class="actions"><button class="btn-verify" onclick="verifyUser(\\''+u.id+'\\')">توثيق</button><button class="btn-block" onclick="toggleUser(\\''+u.id+'\\')">'+(u.isActive?'حظر':'إلغاء')+'</button><button class="btn-del" onclick="deleteUser(\\''+u.id+'\\')">حذف</button></span></div>'
    ).join('');
    const posts = await (await fetch('/api/posts/feed', { headers: { 'Authorization': 'Bearer ' + token } })).json();
    document.getElementById('admin-posts').innerHTML = posts.slice(0,10).map(p =>
      '<div class="user-row"><span>'+p.text?.substring(0,30)+'</span><span>بواسطة '+p.userId?.username+'</span><button class="btn-del" onclick="deletePost(\\''+p.id+'\\')">حذف</button></div>'
    ).join('');
...
// ========== تشغيل الخادم ==========
server.listen(PORT, () => {
  console.log(`🚀 FaceLove Ultra running on http://localhost:${PORT}`);
  console.log(`📊 Admin: set role to "admin" in database (in memory)`);
});
