...
// ========== قاعدة بيانات في الذاكرة (بدون MongoDB) ==========
const db = {
  users: [],
  posts: [],
  messages: [],
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (findUserByUsername(username)) return res.status(400).json({ error: 'Username already used' });
...
app.post('/api/quiz/submit', auth, (req, res) => {
  try {
    ...
    const existing = db.quizzes.findIndex(q => q.userId === req.user.id);
    if (existing > -1) db.quizzes.splice(existing, 1);
    db.quizzes.push({ userId: req.user.id, answers });
    const results = [];
    for (let q of db.quizzes) {
      if (q.userId === req.user.id) continue;
...
app.get('/api/chat/users', auth, (req, res) => {
  const msgUsers = db.messages
    .filter(m => m.senderId === req.user.id || m.receiverId === req.user.id)
    .map(m => m.senderId === req.user.id ? m.receiverId : m.senderId);
  const unique = [...new Set(msgUsers)];
  const users = db.users.filter(u => unique.includes(u.id) && u.id !== req.user.id);
...
<input type="text" id="comment-input-\${p.id}" placeholder="أضف تعليقاً..." style="width:100%;padding:8px 12px;border-radius:30px;border:1px solid #eee;margin-top:6px" onkeypress="if(event.key==='Enter')addComment('\${p.id}')">
</div>
</div>\`).join('');
...
c.innerHTML=msgs.map(m=>\`<div class="message \${m.senderId?._id===currentUser.id?'sent':''}">\${m.senderId?._id===currentUser.id?'':'<strong>'+m.senderId?.username+':</strong> '}\${m.text}</div>\`).join('');
...
document.getElementById('admin-users').innerHTML=users.map(u=>'<div class="user-item"><span>'+u.username+(u.verified?' ✅':'')+'</span><span>'+(u.isActive?'نشط':'محظور')+'</span><span class="actions"><button class="btn-verify" onclick="verifyUser(\\''+u.id+'\\')">توثيق</button><button class="btn-block" onclick="toggleUser(\\''+u.id+'\\')">'+(u.isActive?'حظر':'إلغاء')+'</button><button class="btn-delete" onclick="deleteUser(\\''+u.id+'\\')">حذف</button></span></div>').join('');
const posts=await(await fetch('/api/posts/feed',{headers:{'Authorization':'Bearer '+token}})).json();
document.getElementById('admin-posts').innerHTML=posts.slice(0,10).map(p=>'<div class="user-item"><span>'+p.text?.substring(0,30)+'</span><span>بواسطة '+p.userId?.username+'</span><button class="btn-delete" onclick="deletePost(\\''+p.id+'\\')">حذف</button></div>').join('');
